...
exports.run = (client, message, args) => {
    ...
    let user = message.mentions.users.first();
      if (message.mentions.users.size < 2) return message.channel.send(new Discord.RichEmbed().setColor('RANDOM').setTitle('Efkar VS Ye Hazırmısın').setDescription(message.author.tag + ', kullanım: efkar-vs <@kullanıcı> <@kullanıcı> .').setFooter('', client.user.avatarURL).setTimestamp());
